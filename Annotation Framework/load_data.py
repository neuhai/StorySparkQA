    ...
    for c, n in enumerate(k):
        if '-story.csv' in n:
            ...
            for index, row in story.iterrows():
                ...
                all_section_counter.append(str(c) + '_' + str(index))



file = open('all_stories.json', 'w')
json.dump(all_stories, file)

file = open('all_titles.json', 'w')
json.dump(all_titles, file)

file = open('all_section_counter.json', 'w')
